...
The registry lists the axes, one entry per axis. It is domain-agnostic — axes may cover any body of the target — so it lives at the harness top level, linked from the harness index. An entry declares four things:
...
Gardening axes are harness-declared facts about the target — distinct from any review or evaluation axes a workflow product declares, which are methodology and travel with the pass.

## Why

Growth is the default state of any tended body — code, tests, and docs all accrete weight and drift from their standards unless something recurringly measures them — and the registry is where a project states what counts as healthy for each, with the mandatory measurement turning the answer into a trend rather than an impression.
...
- [`./enforcement-channels.md`](./enforcement-channels.md) — `canon:enforcement-channels`: axes are the judgment channel where prose standards get their judge.
